...
var boundingboxfield = document.getElementById("boundingbox")
  ? document.getElementById("boundingbox").value
  : "";
...
modify.createOrUpdateVertexFeature_ = function (coordinates) {
...
modify.handleUpEvent_ = function (evt) {
  ...
  if (modify.modified_) {
    modify.dispatchEvent(
      new ol.interaction.ModifyEvent(
        ol.ModifyEventType.MODIFYEND,
        this.features_,
        evt,
      ),
    );
...
var centerSelection = function () {
  // center and resize select-box fitting the current view
  var factorX = 0.3;
  var factorY = 0.3;
...
var centerSelectionControl = function (opt_options) {
  ...
  button.title =
    "Auswahl-Rechteck zurücksetzen und auf Karte neu positionieren";
...
const mapconfig = document.getElementById("map").dataset;
...
var map = new ol.Map({
  ...
  view: new ol.View(
    mapconfig.useOsm === "true"
      ? {
          center: ol.proj.fromLonLat([9, 48.66]),
          zoom: 8,
          minZoom: 6,
          maxZoom: 20,
        }
...
map.once("postrender", function () {
  // wait for map to be mapIsInitialized
...
if (!bboxpredefined) {
  // no location service when we already have a box given
  ...
  geolocation.once("change", function (evt) {
    ...
    if (mapIsInitialized) {
      // make sure the map is initialized before calling centerselection
...
window.onAutocompleteListItemSelect = function (e) {
  var bbox = getMapAdapter().prepareBBox(e);
