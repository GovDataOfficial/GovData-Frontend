...
import { SortableData, SortableDataOption, useSortableData } from "../hooks/useSortableData";
...
const columns: SortableDataOption<TestData>[] = [
    { key: 'name' },
    { key: 'age', sort: (a, b, direction) => direction === 'ascending' ? +a - +b : +b - +a },
];
...
describe('useSortableData', () => {
    ...
    it('should sort data with an initial sort configuration', () => {
        const { result } = renderHook(() => useSortableData(data, columns, { key: 'age', direction: 'descending' }));
