...
import {
  ...
  boundingBoxStringCoordinates,
  MappedSuggest,
} from "@/types/types";

export type LocationSearchAutocomplete = {
  sessionId?: string;
  onSearchItemSelected: (suggestion: MappedSuggest) => void;
};
...
export type BaseMapSuggestResponse = {
  ...
  features: {
    id: string;
    bbox: boundingBoxNumberCoordinates;
    properties: { text: string; typ: string };
  }[];
};

export type OSMSuggestResponse = {
  boundingbox: boundingBoxStringCoordinates;
  display_name: string;
  osm_id: number;
  osm_type: string;
}[];
...
export function LocationSearchAutocomplete({
  ...
  const zoomToSelectedLocation = (suggestion: MappedSuggest) => {
    onSearchItemSelected(suggestion);
